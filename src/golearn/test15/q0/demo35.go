...
func main() {
	...
	_ = &([]int{1, 2, 3}[0]) //对切片字面量的索引结果值 是 可寻址的
	//_ = &([]int{1, 2, 3}[0:2]) //对切片字面量的切片结果值 不可寻址
	...
	//_ = &(func(x, y int) {return x + y}) //字面量代表的函数不可寻址
	//_ = &(fmt.Sprintf()) //表示符代表的函数不可寻
	//_ = &(fmt.Sprintln("abc")) //对函数的调用结果不可寻址

	dog := Dog{"little pig"}
	_ = dog
	//_ = &(dog.Name)// 标示符代表的函数不可寻址
