...
func main() {
	...
	if file3 != nil{
		file3.WriteString("The Go language propram writes something to stderr.\n")
	}
	fmt.Println()
	...
	fmt.Printf("Is there only on file descripter for the same file in the same process? %v\n", file5a.Fd() == file4.Fd())
	file5b := os.NewFile(file5a.Fd(), filePath1)
	fmt.Printf("Can the same file descriptor represent the same file? %v\n", file5a.Name() == file5b.Name())
	fmt.Println()
